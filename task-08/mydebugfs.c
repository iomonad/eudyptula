...
static ssize_t id_read(struct file *fp, char __user *user,
		       size_t size, loff_t *loff) {
    return 0;
}

static ssize_t id_write(struct file *fp, const char __user *buffer,
			size_t size, loff_t *loff) {
    return 0;
}
...
static ssize_t jiffries_read(struct file *fp, char __user *user,
			     size_t size, loff_t *loff) {
    return 0;
}
...
static struct file_operations jiffries_fops = {
    .open = jiffries_open,
    .read = jiffries_read,
...
static __init int initialize(void)
{
    ...
    if ((id = debugfs_create_file("id", 777, mentry, NULL, &id_fops)) == NULL)
	return 1;
    if ((jiffries = debugfs_create_file("jiffries", 777, mentry, NULL, &jiffries_fops))
	== NULL)
    	return 1;
    if ((foo = debugfs_create_file("foo", 777, mentry, NULL, &foo_fops)) == NULL)
	return 1;
    return 0;
}
